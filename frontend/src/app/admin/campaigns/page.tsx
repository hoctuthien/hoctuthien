  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Quản lý Chiến dịch thiện nguyện</h1>
          <p className="text-slate-500 text-sm mt-1">Tạo và quản lý các chiến dịch gây quỹ</p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-xs text-slate-400 uppercase tracking-wide">
                ...
                <th className="text-right p-4">Mục tiêu</th>
                <th className="text-right p-4">Đã thu</th>
            ...
            <tbody>
              {campaigns.length === 0 && (
                <tr><td colSpan={5} className="text-center py-10 text-slate-400 text-sm">Chưa có chiến dịch nào</td></tr>
              )}
              {campaigns.map((c: any) => (
                <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  <td className="p-4 font-semibold text-slate-800">{c.title}</td>
                  <td className="p-4 text-right text-slate-600">{Number(c.targetAmount).toLocaleString("vi-VN")}đ</td>
                  <td className="p-4 text-right text-emerald-600 font-bold">{Number(c.raisedAmount || 0).toLocaleString("vi-VN")}đ</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl w-full max-w-lg mx-4 p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h2 className="font-black text-slate-900">{editing.id ? "Cập nhật chiến dịch" : "Tạo chiến dịch mới"}</h2>
            ...
            {[
              ...
              { label: "URL ảnh bìa", key: "thumbnailUrl", type: "text" },
              { label: "Mục tiêu (đồng)", key: "targetAmount", type: "number" },
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs font-black text-slate-500 uppercase tracking-wide">Trạng thái</label>
              <select
            ...
            <div className="flex gap-3 justify-end mt-2">
              <button onClick={() => setFormOpen(false)} className="px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-700 cursor-pointer border-0 bg-transparent">Hủy</button>
              <button
                onClick={handleSave}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl w-full max-w-sm mx-4 p-6 flex flex-col gap-4">
            <h2 className="font-black text-slate-900">Xác nhận xóa</h2>
            <p className="text-sm text-slate-500">Bạn có chắc muốn xóa chiến dịch này không? Hành động này không thể hoàn tác.</p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setDeleteId(null)} className="px-4 py-2 text-sm font-bold text-slate-500 cursor-pointer border-0 bg-transparent">Hủy</button>
