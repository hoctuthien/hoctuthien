...
export function PostFilter({ categories, tags }: PostFilterProps) {
  const tExtracted = useTranslations('Extracted.appAdminPostsComponentsPostFilter');
  ...
  const categoryOptions = [
    { label: tExtracted('allCategories'), value: "" },
    ...categories.map(c => ({ label: c.name, value: c.id }))
  ];

  const tagOptions = [
    { label: tExtracted('allTags'), value: "" },
    ...
    <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-center">
      <div className="relative flex-1 group">
        ...
        <input
          ...
          placeholder={tExtracted('searchPostsByTitle')}
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
        <div className="w-full sm:w-48">
          <Select
            ...
            placeholder={tExtracted('category')}
