...
export default function AdminEditPostPage() {
  const tExtracted = useTranslations('Extracted.appAdminPostsIdEditPage');
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        setThumbnail(post.metadata?.thumbnail || post.thumbnail || "");
  ...
  const handleSave = async (newStatus: string) => {
    ...
    try {
      ...
      alert(tExtracted('errorUpdatingPost'));
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-sm text-slate-400">{tExtracted('loadingPost')}</span>
        </div>
      </div>
    );
  }
      ...
      <div className="h-full flex flex-col -m-6 lg:-m-10 bg-white">
        ...
        <div className="h-16 border-b border-slate-100 px-6 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button label={status === "published" ? tExtracted('update') : tExtracted('publish')} variant="primary" className="!px-6 !py-2 !rounded-xl !text-xs shadow-lg shadow-primary/20" loading={isLoading} onClick={() => handleSave("published")} />
          </div>
        </div>
