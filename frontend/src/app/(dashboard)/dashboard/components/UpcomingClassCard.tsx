...
export const UpcomingClassCard: React.FC<UpcomingClassCardProps> = ({ isMentor, allBookings }) => {
  const tExtracted = useTranslations('Extracted.appDashboardDashboardComponentsUpcomingClassCard');
  ...
  return (
    <div className="bg-white border border-[#E2E8F0] rounded-[32px] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.005)] flex flex-col gap-5">
      <div className="flex flex-col gap-1 border-b border-slate-100 pb-3">
        <span className="text-[9px] font-black text-[#005BBF] uppercase tracking-widest">{tExtracted('thoiGianBieuHomNay')}</span>
        <h3 className="text-xs font-black text-[#0F172A] font-[Montserrat]">
          {isMentor ? tExtracted('buoiDaySapToi') : tExtracted('lopHocTiepTheo')}
        </h3>
      </div>
      ...
      {nextBooking ? (
          ...
          return (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-[8px] text-[#94A3B8] font-black uppercase tracking-wider">{tExtracted('khoaHoc')}</span>
              ...
              {isConfirmed && nextBooking.googleMeetUrl ? (
                <a
                  ...
                  className="bg-[#005BBF] hover:bg-[#004493] text-white font-extrabold text-[10px] py-3 rounded-xl uppercase tracking-wider text-center no-underline shadow-md shadow-[#005BBF]/10 transition-all flex items-center justify-center gap-1.5 cursor-pointer mt-2"
                >
                  <LuVideo size={14} />
                  <span>{tExtracted('vaoGoogleMeet')}</span>
                </a>
              ) : (
                <div className="bg-slate-50 text-[#64748B] border border-slate-200 font-extrabold text-[9px] py-3 rounded-xl uppercase tracking-wider text-center block mt-2">
                  {isConfirmed ? tExtracted('choLinkPhongHoc') : tExtracted('choCoVanXacNhanLop')}
                </div>
              )}
            </div>
          );
        ...
        <div className="text-center py-6 text-[11px] font-semibold text-[#94A3B8] italic">
          {tExtracted('khongCoLichHocNaoSapToi')}</div>
      )}
    </div>
  );
};
