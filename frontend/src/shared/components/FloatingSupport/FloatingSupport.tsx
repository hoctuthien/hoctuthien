...
export function FloatingSupport() {
  const tExtracted = useTranslations('Extracted.sharedComponentsFloatingSupportFloatingSupport');
    ...
    <>
      <div className="fixed bottom-5 right-5 z-40 flex flex-col items-end gap-3">
          ...
          <div className="flex flex-col items-end gap-2 animate-in fade-in slide-in-from-bottom-2 duration-200">
            {isLoggedIn && (
              <button
                ...
                aria-label={tExtracted('baoLoiHeThong')}
              >
                <span className="text-xs font-black">{tExtracted('baoLoi')}</span>
            ...
            <a
              ...
              aria-label={tExtracted('lienHeQuaEmail')}
            >
              <span className="text-xs font-black">{tExtracted('email')}</span>
            ...
            <button
              ...
              aria-label={tExtracted('lienHeQuaChat')}
            >
              <span className="text-xs font-black">{tExtracted('lienHe')}</span>
        ...
        <button
          ...
          aria-label={isOpen ? tExtracted('dongHoTro') : tExtracted('moHoTro')}
