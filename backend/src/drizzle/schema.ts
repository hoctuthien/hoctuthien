import { pgTable, bigserial, timestamp, varchar, uuid, boolean, unique, date, integer, jsonb, foreignKey, bigint, text, inet, numeric, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const courseBookingsStatusEnum = pgEnum("course_bookings_status_enum", ['pending', 'confirmed', 'completed', 'cancelled', 'rescheduled'])
export const coursesStatusEnum = pgEnum("courses_status_enum", ['DRAFT', 'PENDING', 'ACTIVE', 'INACTIVE'])
export const mentorAvailabilitiesStatusEnum = pgEnum("mentor_availabilities_status_enum", ['PENDING', 'IN_PROGRESS', 'APPROVED', 'REJECTED'])
export const paymentsStatusEnum = pgEnum("payments_status_enum", ['pending', 'success', 'failed', 'expired'])
export const usersRoleEnum = pgEnum("users_role_enum", ['mentee', 'mentor', 'admin'])
...
export const courseCategories = pgTable("course_categories", {
	...
	unique("UQ_0fcee24a5291cf6c3de60ea670e").on(table.courseId, table.categoryId),
]);
...
export const categories = pgTable("categories", {
	...
	unique("UQ_420d9f679d41281f282f5bc7d09").on(table.slug),
]);
...
export const payments = pgTable("payments", {
	...
	paymentGatewayPayload: jsonb("payment_gateway_payload").default({}).notNull(),
...
export const penaltyTickets = pgTable("penalty_tickets", {
	id: bigserial({ mode: "bigint" }).primaryKey().notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow().notNull(),
	deletedAt: timestamp("deleted_at", { mode: 'string' }),
